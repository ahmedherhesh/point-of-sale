...
import { totalPrice, cartEls, saleForm } from '../../main';
...
const addToCart = e => {
    ...
    if (!cartEls.includes(itemId)) {
        cartEls.push(itemId)
        const salePrice = saleForm.is_whole_sale == true ? el.dataset.wholeSalePrice : el.dataset.salePrice;
        tbody.innerHTML += ` <tr class='cart-item' data-id="${itemId}">
                                <td class='text-center'>${el.dataset.title}</td>
                                <td class='price text-center'>${salePrice}</td>
                                <td class='counter'>
                                ...
                                <td class='total-price text-center'>${salePrice}</td>
...
<template>
    ...
    <div v-if="items.data.length > 0"
        ...
        <div v-for="item in items.data" @click="addToCart"
            ...
            :data-title="item.title" :data-sale-price="item.sale_price" :data-whole-sale-price="item.whole_sale_price" :data-stock="item.stock">
